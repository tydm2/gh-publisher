...
- **🌍 ترجمات موفّرة للرموز** — تُحدَّث ترجمات README متعددة اللغات عبر **الترجمة بالتعديل الإضافي (incremental-edit translation)** (يحرّر الوكلاء الفرعيون الفقرات المتغيرة فقط فوق الترجمة القديمة، دون إعادة ترجمة كاملة): توفير نحو 70-90% من رموز الإخراج، وأسرع، مع بقاء المصطلحات والأسلوب مستقرين.
...
- **مخطط ثابت موفّر للرموز**: اللغات التي لديها ترجمة موجودة تُحدَّث عبر الترجمة بالتعديل الإضافي (تحرير الفقرات المتغيرة فقط — دون إعادة ترجمة كاملة)؛ أما اللغات الجديدة أول مرة فتحصل على ترجمة كاملة واحدة تُرسي خط الأساس للمصطلحات. راجع `references/i18n.md` §1.5.

## النتائج المقاسة: v1.4.0 → v1.5.0 (الترجمة بالتعديل الإضافي)

كان تحديث README لـ 11 لغة في هذا الإصدار أول تشغيل لمخطط التعديل الإضافي. قِسناها مقابل إعادة الترجمة الكاملة السابقة، باستخدام ملفات v1.4.0 الفعلية (المجلوبة من المستودع عند الالتزام `40dcf1f`) مقابل ملفات v1.5.0 المُصدرة (فرق على مستوى الأسطر):

| المقياس | إعادة الترجمة الكاملة (عصر v1.4) | التعديل الإضافي (v1.5) | الموفَّر |
|---|---|---|---|
| محتوى الإخراج، 11 لغة | 68,292 حرفًا | 7,717 حرفًا | **88.7%** |
| رموز الإخراج التقريبية (~3 أحرف/رمز) | ~22,800 | ~2,600 | **~88.7%** |
| المصطلحات / الأسلوب | قد تنحرف مع كل إصدار | الترجمات القديمة متطابقة بايتًا ببايت عدا الفقرات المتغيرة | مستقر |
| الجهد لكل لغة | إعادة توليد الملف بالكامل | ~4 تعديلات مستهدفة | أسرع |

نسبة التغير لكل لغة (الأحرف المتغيرة / الملف القديم الكامل): zh-CN 9.1% · hi 10.8% · es 11.9% · fr 12.5% · ar 12.2% · bn 11.3% · pt 11.6% · ru 12.0% · ja 8.9% · de 12.3% · ko 9.4% — **تغيّر فعليًا نحو 9-12% فقط من كل ملف**، لذا أنفقت إعادة الترجمة الكاملة نحو 8-11 أضعاف الإخراج اللازم.

*المنهجية: ملفات v1.4.0 مجلوبة من المستودع عند الالتزام `40dcf1f`؛ فرق على مستوى الأسطر لكل لغة (الأسطر المضافة/المعدلة) مقابل ملفات v1.5.0 المُصدرة؛ الرموز مُقدَّرة بـ ~3 أحرف/رمز لملفات README متعددة اللغات. تختلف تكاليف جانب الإدخال أيضًا (يقرأ التعديل الإضافي الترجمة القديمة)، لكن الإخراج هو الجزء المهيمن والأكثر تكلفة في ترجمة LLM.*
...
- `references/i18n.md` — بروتوكول النشر متعدد اللغات (10 لغات، الترجمة بالتعديل الإضافي)
