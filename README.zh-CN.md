...
`gh-publisher` 是一个智能体技能（agent skill），使用 `gh` CLI + GitHub REST API（Contents / Git Database）将本地目录发布到 GitHub 仓库——无需 git。它用一个可复用的脚本处理空仓库初始化与批量提交，让智能体一条命令完成推送，而不必重新推导整套 API 流程。它还会发布 **6 种发布语言（en、zh-CN、es、fr、ja、ru）** 的 README 翻译版，而本地工作副本始终保留在你偏好的语言。
...
- **🌍 多语言发布** —— 本地工作副本保持在你所选的语言（默认中文）；发布版随附 6 种发布语言（en、zh-CN、es、fr、ja、ru）的 README 翻译。参见 `references/i18n.md`。
...
8. 可选 `-Languages` 检查：推送前逐一验证每个语言文件是否存在（例如 `README.es.md`），缺失时给出警告。

## 多语言发布（本地语言 + 6 种发布语言）

- **本地工作副本**：保持在你配置的语言（zh 或 en）——即已安装技能目录下的 `config.local.json`（默认 `zh`）。说一句 *"把本地默认语言改成英文"* 即可切换；本地 SKILL.md 会同步更新。
- **发布版**：`SKILL.md` 保持英文（GitHub 的通用主语言），同时发布 6 种发布语言（en、zh-CN、es、fr、ja、ru）的 `README.<lang>.md`。语言列表、翻译规则与触发契约注意事项参见 `references/i18n.md`（所有语言版本保持相同的 `name`）。
- **省 token 的固定方案**：已有翻译的语言通过增量编辑翻译更新（只编辑发生变化的段落——不做全文重翻）；首次翻译的语言做一次全文翻译，以建立术语基线。参见 `references/i18n.md` §1.5。

## 实测结果：v1.4.0 → v1.5.0（增量编辑翻译）

本版本的 11 种语言 README 更新是增量编辑方案的首次运行。我们将其与此前的全文重翻进行了对比测量：使用仓库中提交 `40dcf1f` 拉取的真实 v1.4.0 文件，与已发布的 v1.5.0 文件做逐行 diff：

| 指标 | 全文重翻（v1.4 时代） | 增量编辑（v1.5） | 节省 |
|---|---|---|---|
| 输出内容（11 种语言） | 68,292 字符 | 7,717 字符 | **88.7%** |
| 估算输出 token（约 3 字符/token） | ~22,800 | ~2,600 | **~88.7%** |
| 术语 / 风格 | 每个版本都可能漂移 | 旧翻译除变化段落外逐字节一致 | 稳定 |
| 每种语言的工作量 | 重新生成整个文件 | 约 4 处定向编辑 | 更快 |

各语言的变化比例（变化字符数 / 旧文件全文）：zh-CN 9.1% · hi 10.8% · es 11.9% · fr 12.5% · ar 12.2% · bn 11.3% · pt 11.6% · ru 12.0% · ja 8.9% · de 12.3% · ko 9.4% —— **每个文件实际只有约 9-12% 发生变化**，因此全文重翻多花费了约 8-11 倍的输出。

*方法：v1.4.0 文件从仓库的提交 `40dcf1f` 拉取；与已发布的 v1.5.0 文件做各语言逐行 diff（新增/修改的行）；混合语言 README 的 token 按约 3 字符/token 估算。输入侧成本也不同（增量编辑需要读取旧翻译），但输出是 LLM 翻译中占主导且最昂贵的部分。*
...
- `references/i18n.md` —— 多语言发布协议（6 种语言，增量编辑翻译）
